...
| 14 | `get_trackzs_etf_info` | `scripts/get_trackzs_etf_info.py` | 通过 | `950162.CSI` 已实测可返回有效 ETF 列表；`000300.CSI` 在当前环境下可能返回空 JSON |
...
- `get_trackzs_etf_info` 当前推荐使用 `950162.CSI` 作为验证样例；`000300.CSI` 在 2026-06-25 的真实 TQ 冒烟中返回 `{}`，并伴随 `server return none`。
